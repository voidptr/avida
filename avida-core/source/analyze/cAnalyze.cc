...
/*
 ...
 command. @kgn
 */
        , temporary_next_id(0), temporary_next_update(0), batch(INITIAL_BATCHES), variables(26), local_variables(26),
          arg_variables(26), exit_on_error(true), m_world(world), m_ctx(world->GetDefaultContext()), m_jobqueue(world),
          m_resources(NULL), m_resource_time_spent_offset(0), interactive_depth(0) {
...
void cAnalyze::LoadResources(cString cur_string) {
    ...
    if (!m_resources->LoadFile(filename, m_world->GetWorkingDir()))
        cerr << "error: failed to load resource file" << endl;
}
...
double cAnalyze::AnalyzeEntropy(cAnalyzeGenotype *genotype, double mu) {
    ...
    const int num_insts = m_world->GetHardwareManager().GetInstSet(
            base_genome.Properties().Get("instset").StringValue()).GetSize();
        ...
        while (1) {
            ...
            for (int mod_inst = 0; mod_inst < num_insts; mod_inst++) {
                ...
                prob[mod_inst] = (mut_rate * w_bar) /
                                 (double_num_insts - 2) /
                                 (w_bar +
                                  test_fitness[mod_inst] * mut_rate * (double_num_insts - 1) / (double_num_insts - 2)
...
double cAnalyze::IncreasedInfo(cAnalyzeGenotype *genotype1,
    ...
    const int num_insts = m_world->GetHardwareManager().GetInstSet(
            genotype1_base_genome.Properties().Get("instset").StringValue()).GetSize();
...
void cAnalyze::CommandFilter(cString cur_string) {
    ...
    tDataEntryCommand<cAnalyzeGenotype> *stat_command = cAnalyzeGenotype::GetDataCommandManager().GetDataCommand(
            stat_name);
...
void cAnalyze::SampleOffspring(cString cur_string) {
    ...
    if (cur_string.CountNumWords() > 0) {
        if (cur_string.CountNumWords() == m_world->GetEnvironment().GetInputSize()) {
            ...
            cerr << "warning: Invalid number of environment inputs requested for recalculation: "
                 << cur_string.CountNumWords()
    ...
    if (m_world->GetVerbosity() >= VERBOSE_ON) {
        msg.Set("Sampling %d offspring from each of the %d organisms in batch %d...", number_to_sample,
                batch[cur_batch].GetSize(), cur_batch);
        cout << msg << endl;
    } else {
    ...
    while ((parent_genotype = batch_it.Next())) {
        ...
        for (int i = 0; i < number_to_sample; i++) {
            ...
            if (found) {
                ...
                cAnalyzeGenotype *offspring_genotype = new cAnalyzeGenotype(m_world, test_info.GetTestOrganism(
                        0)->OffspringGenome());
...
void cAnalyze::CommandDetail(cString cur_string) {
    ...
    if (filename == "cout") {
        ...
        Avida::Output::FilePtr df = Avida::Output::File::CreateWithPath(m_world->GetNewWorld(),
                                                                        (const char *) filename);
        ofstream &fp = df->OFStream();
...
void cAnalyze::CommandDetailTimeline(cString cur_string) {
    ...
    if (filename == "cout") {
        ...
        Avida::Output::FilePtr df = Avida::Output::File::StaticWithPath(m_world->GetNewWorld(),
                                                                        (const char *) filename);
        ofstream &fp = df->OFStream();
...
void cAnalyze::CommandDetailBatches(cString cur_string) {
    ...
    tDataEntryCommand<cAnalyzeGenotype> *cur_command = cAnalyzeGenotype::GetDataCommandManager().GetDataCommand(
            keyword);
    if (!cur_command) {
    ...
    for (int i = 0; i < GetNumBatches(); i++) {
        ...
        while ((genotype = batch_it.Next()) != NULL) {
            ...
            if (file_type == FILE_TYPE_HTML) {
                HTMLPrintStat(cur_command->GetValue(genotype), fp, 0, cur_command->GetHtmlCellFlags(),
                              cur_command->GetNull());
...
void cAnalyze::CommandPrintPhenotypes(cString cur_string) {
    if (m_world->GetVerbosity() >= VERBOSE_ON)
        cout << "Printing phenotypes in batch "
             << cur_batch << endl;
    ...
    while ((genotype = batch_it.Next()) != NULL) {
        ...
        for (int i = 0; i < num_tasks; i++) {
            if (genotype->GetTaskCount(i) > 0)
                phen_id.Set(i + 1, true);  // again, +1 because we used 0th bit for viability
        }
    ...
    for (Apto::Map<cBitArray, p_stats>::ValueIterator it = phenotype_table.Values(); it.Next();)
        phenotype_array.Push(*it.Get());
    Apto::QSort(phenotype_array, Apto::Functor<int, Apto::TL::Create<const p_stats &, const p_stats &> >(
            &cAnalyze::PStatsComparator)); // sort by cpu_count, greatest to least
...
void cAnalyze::CommandPrintDiversity(cString cur_string) {
    if (m_world->GetVerbosity() >= VERBOSE_ON)
        cout << "Printing diversity data for batch "
             << cur_batch << endl;
    ...
    for (int task_id = 0; task_id < num_tasks; task_id++) {
        ...
        cAnalyzeGenotype *genotype = NULL;
        while ((genotype = batch_it.Next()) != NULL) {
            if (m_world->GetHardwareManager().GetInstSet(
                    genotype->GetGenome().Properties().Get("instset").StringValue()).GetInstSetName() !=
                is.GetInstSetName() || genotype->GetTaskCount(task_id) == 0)
                continue;
        ...
        for (int line = 0; line < length_genome; ++line) {
            ...
            for (int inst = 0; inst < num_insts; ++inst) {
                ...
                if (prob_before_env[line][inst] > 0) {
                    entropy[line] -=
                            prob_before_env[line][inst] * log(prob_before_env[line][inst]) / log(num_insts * 1.0);
                }
            }
...
void cAnalyze::CommandPrintTreeStats(cString cur_string) {
    if (m_world->GetVerbosity() >= VERBOSE_ON)
        cout << "Printing tree stats for batch "
             << cur_batch << endl;
...
void cAnalyze::CommandPrintCumulativeStemminess(cString cur_string) {
    if (m_world->GetVerbosity() >= VERBOSE_ON)
        cout << "Printing cumulative stemmines for batch "
             << cur_batch << endl;
...
void cAnalyze::CommandPrintGamma(cString cur_string) {
    if (m_world->GetVerbosity() >= VERBOSE_ON)
        cout << "Printing Pybus-Harvey gamma statistic for batch "
             << cur_batch << endl;
  ...
  */
    ...
    if (lineage_thru_time_fname != "") {
        Avida::Output::FilePtr df = Avida::Output::File::StaticWithPath(m_world->GetNewWorld(),
                                                                        (const char *) lineage_thru_time_fname);
...
void cAnalyze::AnalyzeCommunityComplexity(cString cur_string) {
    ...
    if (max_genotypes > 0) {
        ...
        while (((genotype = batch_it.Next()) != NULL) &&
               (community.size() < static_cast<unsigned int>(max_genotypes))) {
            community.push_back(genotype);
        }
    ...
    const int num_insts = m_world->GetHardwareManager().GetInstSet(
            cur_genome.Properties().Get("instset").StringValue()).GetSize();
    for (int line = 0; line < length_genome; ++line) {
...
void cAnalyze::CommandPrintResourceFitnessMap(cString cur_string) {
    ...
    for (int i = 0; i < m_world->GetEnvironment().GetResourceLib().GetSize(); i++) {
        ...
        for (int j = 0; j < m_world->GetEnvironment().GetReactionLib().GetSize(); j++) {
            if (m_world->GetEnvironment().GetReactionLib().GetReaction(j)->GetProcesses().GetPos(
                    0)->GetResource()->GetName() == name) {
...
void cAnalyze::AnalyzeComplexityDelta(cString cur_string) {
    ...
    for (int cur_test = 0; cur_test < num_tests; cur_test++) {
        ...
        const cInstSet &inst_set = m_world->GetHardwareManager().GetInstSet(
                mod_genome.Properties().Get("instset").StringValue());
...
void cAnalyze::AnalyzeKnockouts(cString cur_string) {
    ...
    while ((genotype = batch_it.Next()) != NULL) {
        ...
        Instruction null_inst = m_world->GetHardwareManager().GetInstSet(
                base_genome.Properties().Get("instset").StringValue()).ActivateNullInst();
...
void cAnalyze::GetSkeletons_Expanded(cString cur_string)
{
  cout << "Turning population into skeletons of only informative sites..." << endl;
...
void cAnalyze::CommandMapTasks(cString cur_string) {
    ...
    while ((genotype = batch_it.Next()) != NULL) {
        ...
        cString filename;
        if (file_type == FILE_TYPE_TEXT) {
            filename.Set("%stasksites.%s.dat", static_cast<const char *>(directory),
                         static_cast<const char *>(genotype->GetName()));
        } else {   //  if (file_type == FILE_TYPE_HTML) {
            filename.Set("%stasksites.%s.html", static_cast<const char *>(directory),
                         static_cast<const char *>(genotype->GetName()));
        }
        Avida::Output::FilePtr df = Avida::Output::File::CreateWithPath(m_world->GetNewWorld(),
                                                                        (const char *) filename);
        ...
        if (file_type == FILE_TYPE_TEXT) {
            ...
            fp << "<body>" << endl
               ...
               << "<h1 align=\"center\">Run " << batch[cur_batch].Name() << ", ID " << genotype->GetID() << "</h1>"
               << endl
               << endl;
        ...
        for (int line_num = 0; line_num < max_line; line_num++) {
            ...
            while ((data_command = output_it.Next()) != NULL) {
                ...
                int compare = CompareFlexStat(test_value, data_command->GetValue(genotype),
                                              data_command->GetCompareType());
        ...
        if (file_type == FILE_TYPE_HTML) {
            ...
            for (int i = 0; i < num_cols; i++) {
                if (col_pass_count[i] > 0) {
                    ...
                    fp << "<th bgcolor=\"#" << m_world->GetConfig().COLOR_MUT_LETHAL.Get() << "\">"
                       << col_fail_count[i];
                } else fp << "<th>0";
            }
            fp << "</tr>" << endl;
            ...
            if (total_task > 1) {
                for (int i = 0; i < num_cols; i++) {
                    ...
                    if (task_overlap(i, i) != 0) {
                        sum_task_overlap =
                                sum_task_overlap + overlap_per_task / (task_overlap(i, i) * (total_task - 1));
                    }
                }
            }
...
void cAnalyze::CommandAnalyzeModularity(cString cur_string) {
    ...
    while ((genotype = batch_it.Next()) != NULL) {
        ...
        const Instruction null_inst = m_world->GetHardwareManager().GetInstSet(
                base_genome.Properties().Get("instset").StringValue()).ActivateNullInst();
...
void cAnalyze::CommandAnalyzeRedundancyByInstFailure(cString cur_string) {
    ...
    while ((genotype = batch_it.Next()) != NULL) {
        ...
        const Genome &genome = genotype->GetGenome();
        const cInstSet &original_inst_set = m_world->GetHardwareManager().GetInstSet(
                genome.Properties().Get("instset").StringValue());
        cInstSet *modify_inst_set = new cInstSet(original_inst_set);
        Apto::String isname =
                genotype->GetGenome().Properties().Get("instset").StringValue() + ":analyze_redundancy_by_inst_failure";
        ...
        if (num_pr_fail_insts == 0) {
            cerr
                    << "ANALYZE_REDUNDANCY_BY_INST_FAILURE: No instructions have a chance of failure in default instruction set."
                    << endl;
            exit(1);
        }
        ...
        if (baseline_fitness > 0) {
            ...
            for (double log10_fc = log10_start_pr_fail;
                 log10_fc <= log10_end_pr_fail; log10_fc += log10_step_size_pr_fail) {
    ...
    while ((genotype = batch_it.Next()) != NULL) {
        ...
        for (unsigned int i = 0; i < fragments[cell_id].size(); i++) {
            ...
            if (m_world->GetConfig().HGT_RECOMBINATION_ALTERNATIVE_EFFECTS.Get() == 3 ||
                ...
                int ins_del = (m_ctx.GetRandom().GetInt((int) (frag_size / ratio), (int) (frag_size * ratio))) - frag_size;

                int point_muts = frag_size;
                ...
                if (ins_del < 0) {
                    ...
                    for (int i = 0; i < num_mut; i++) mut_sites[i] = m_ctx.GetRandom().GetUInt(seq.GetSize() + 1);
                    Apto::QSort(mut_sites);

                    // Actually do the mutations (in reverse sort order)
                    for (int i = mut_sites.GetSize() - 1; i >= 0; i--) {
                        if (isRandomInstruction)
                            seq.Insert(mut_sites[i], is.GetRandomInst(m_ctx));
                        else
                            seq.Insert(mut_sites[i], frag[m_ctx.GetRandom().GetUInt(frag_size)]);
                    }
                ...
                } else if (ins_del > 0) // ok, we're going to do some deletions
                {
                    ...
                    if (seq.GetSize() - num_mut < m_world->GetConfig().MIN_GENOME_SIZE.Get()) {
                        num_mut = seq.GetSize() - m_world->GetConfig().MIN_GENOME_SIZE.Get();
                    }

                    // If we have lines to delete...
                    for (int i = 0; i < num_mut; i++) {
                        int site = m_ctx.GetRandom().GetUInt(seq.GetSize());
                        seq.Remove(site);
                    }
                ...
                for (int i = 0; i < point_muts; i++) {
                    int site = m_ctx.GetRandom().GetUInt(seq.GetSize());
                    if (isRandomInstruction)
                        seq[site] = is.GetRandomInst(m_ctx);
                    else
                        seq[site] = frag[m_ctx.GetRandom().GetUInt(frag_size)];
                }
...
void cAnalyze::CommandMapSingleStepNetwork(cString cur_string) {
    ...
    while ((genotype = batch_it.Next()) != NULL) {
        ...
        // Construct this filename...
        cString filename;
        filename.Set("%smut_map.%s.dat", static_cast<const char *>(directory),
                     static_cast<const char *>(genotype->GetName()));
...
void cAnalyze::CommandMapMutations(cString cur_string) {
    ...
    while ((genotype = batch_it.Next()) != NULL) {
        ...
        if (file_type == FILE_TYPE_TEXT) {
            ...
            filename.Set("%smut_map.%s.html", static_cast<const char *>(directory),
                         static_cast<const char *>(genotype->GetName()));
        }
        if (m_world->GetVerbosity() >= VERBOSE_ON) {
            cout << "  Using filename \"" << filename << "\"" << endl;
        ...
        const cInstSet &inst_set = m_world->GetHardwareManager().GetInstSet(
                base_genome.Properties().Get("instset").StringValue());
        const int num_insts = inst_set.GetSize();
        ...
        for (int line_num = 0; line_num < max_line; line_num++) {
            ...
            if (file_type == FILE_TYPE_HTML)
                fp << "<th bgcolor=\"#" << m_world->GetConfig().COLOR_MUT_NEG.Get() << "\">";
            fp << (double) row_neg / (double) (num_insts - 1) << " ";

            if (file_type == FILE_TYPE_HTML)
                fp << "<th bgcolor=\"#" << m_world->GetConfig().COLOR_MUT_NEUT.Get() << "\">";
...
void cAnalyze::CommandSpecies(cString cur_string) {
    ...
    if (m_world->GetVerbosity() <= VERBOSE_NORMAL) cout << "Calculating Species Distance... " << endl;
        ...
        cout << "Calculating Species Distance between batch "
             << batch1 << " and " << batch2 << endl;
...
void cAnalyze::CommandRecombine(cString cur_string) {
    ...
    if (m_world->GetVerbosity() <= VERBOSE_NORMAL) cout << "Creating recombinants...  " << endl;
        ...
        cout << "Creating recombinants between batch "
             << batch1 << " and " << batch2 << endl;
    ...
    while ((genotype1 = list1_it.Next()) != NULL) {
        ...
        while ((genotype2 = list2_it.Next()) != NULL) {
            ...
            for (int iter = 1; iter < num_compare; iter++) {
                ...
                new_genotype0->SetParentID(
                        genotype1->GetID()); //@CHC: Want to keep track of which two parents generated this offspring
...
void cAnalyze::CommandMutagenize(cString) {
    ...
    while ((genotype = batch_it.Next()) != NULL) {
        ...
        const cInstSet &inst_set = m_world->GetHardwareManager().GetInstSet(
                cur_genome.Properties().Get("instset").StringValue());
...
void cAnalyze::AnalyzeMuts(cString cur_string) {
    ...
    if (scan_combos) {
        ...
        for (int combo_id = 0; combo_id < total_combos; combo_id++) {
            ...
            HashPropertyMap props;
            cHardwareManager::SetupPropertyMap(props, (const char *) is.GetInstSetName());
            Genome test_genome(is.GetHardwareType(), props,
                               GeneticRepresentationPtr(new InstructionSequence((const char *) test_sequence)));
            cCPUTestInfo test_info;
        ...
        for (int i = 0; i <= total_diffs; i++) {
            ...
            HashPropertyMap props;
            cHardwareManager::SetupPropertyMap(props, (const char *) is.GetInstSetName());
            Genome max_genome(is.GetHardwareType(), props,
                              GeneticRepresentationPtr(new InstructionSequence((const char *) max_sequence[i])));
...
void cAnalyze::AnalyzeInstructions(cString cur_string) {
    ...
    if (file_type == FILE_TYPE_HTML) {
        ...
        for (int i = 0; i < num_insts; i++) {
            ...
            else if (inst_freq < max_freq)
                fp << "<td bgcolor=\"#" << m_world->GetConfig().COLOR_MUT_NEUT.Get() << "\">";
...
void cAnalyze::AnalyzeComplexity(cString cur_string) {
    ...
    Avida::Output::FilePtr lineage_df = Avida::Output::File::CreateWithPath(m_world->GetNewWorld(),
                                                                            (const char *) lineage_filename);
    ...
    while ((genotype = batch_it.Next()) != NULL) {
        ...
        // Construct this filename...
        cString filename;
        filename.Set("%s%s.complexity.dat", static_cast<const char *>(directory),
                     static_cast<const char *>(genotype->GetName()));
        Avida::Output::FilePtr df = Avida::Output::File::CreateWithPath(m_world->GetNewWorld(),
                                                                        (const char *) filename);
        ofstream &fp = df->OFStream();
    ...
    while ((genotype = batch_it.Next()) != NULL) {
        ...
        for (int site1 = 0; site1 < max_line; site1++) {
            for (int site2 = site1 + 1; site2 < max_line; site2++) {
                ...
                cString fl_filename;
                fl_filename.Set("%s%s_FitLand_sites-%d_and_%d.dat", static_cast<const char *>(directory),
                                static_cast<const char *>(genotype->GetName()), site1, site2);
                Avida::Output::FilePtr fit_land_fp = Avida::Output::File::CreateWithPath(m_world->GetNewWorld(),
                                                                                         (const char *) fl_filename);
...
// get current batch
    ...
    Avida::Output::FilePtr fp_2s = Avida::Output::File::CreateWithPath(m_world->GetNewWorld(),
                                                                       (const char *) filename_2s);
    ...
    while ((genotype = batch_it.Next()) != NULL) {
        ...
        const int num_insts = m_world->GetHardwareManager().GetInstSet(
                base_genome.Properties().Get("instset")).GetSize();
...
void cAnalyze::AnalyzeComplexityTwoSites(cString cur_string) {
   ...
   */
    ...
    Avida::Output::FilePtr summary_fp = Avida::Output::File::CreateWithPath(m_world->GetNewWorld(),
    ...
    while ((genotype = batch_it.Next()) != NULL) {
        ...
        // Construct filename
        cString filename_2s;
        filename_2s.Set("%s%s.twosite.complexity.dat", static_cast<const char *>(directory),
                        static_cast<const char *>(genotype->GetName()));
        Avida::Output::FilePtr fp_2s = Avida::Output::File::CreateWithPath(m_world->GetNewWorld(),
                                                                           (const char *) filename_2s);
        ...
        for (int line_num1 = 0; line_num1 < max_line; line_num1++) {
            for (int line_num2 = line_num1 + 1; line_num2 < max_line; line_num2++) {
                    ...
                    for (int mod_inst1 = 0; mod_inst1 < num_insts; mod_inst1++) {
                        for (int mod_inst2 = 0; mod_inst2 < num_insts; mod_inst2++) {
                            ...
                            if (mut_rate != 0.0) {
                                // run mutation-selection equation
                                // -term 1
                                prob_next_2s[mod_inst1][mod_inst2] =
                                        ((1.0 - mut_rate) * (1.0 - mut_rate) * test_fitness_2s[mod_inst1][mod_inst2] *
                                         prob_2s[mod_inst1][mod_inst2]) / (w_avg_2s);
                                // -term 2
                                ...
                                prob_next_2s[mod_inst1][mod_inst2] +=
                                        (((mut_rate * (1.0 - mut_rate)) / ((double) num_insts))) * sum_term2;
                                // -term 3
                                ...
                                prob_next_2s[mod_inst1][mod_inst2] +=
                                        (((mut_rate * (1.0 - mut_rate)) / ((double) num_insts))) * sum_term3;
                                // -term 4
                                prob_next_2s[mod_inst1][mod_inst2] +=
                                        (mut_rate / ((double) num_insts)) * (mut_rate / ((double) num_insts));
                            } else {
                                // run selection equation
                                prob_next_2s[mod_inst1][mod_inst2] =
                                        (test_fitness_2s[mod_inst1][mod_inst2] * prob_2s[mod_inst1][mod_inst2]) /
                                        (w_avg_2s);

                            }
                            // increment change in probs
                            delta_prob_2s += (prob_next_2s[mod_inst1][mod_inst2] - prob_2s[mod_inst1][mod_inst2]) *
                                             (prob_next_2s[mod_inst1][mod_inst2] - prob_2s[mod_inst1][mod_inst2]);
                            //delta_prob_ex += (prob_next[mod_inst]-prob[mod_inst]);
                        }
                    }
                ...
                for (int i = 0; i < num_insts; i++) {
                    ...
                    if (prob_1s_i[i] != 0.0) {
                        // mers
                        entropy_ss_site1_mers +=
                                prob_1s_i[i] * log((double) 1.0 / prob_1s_i[i]) / log((double) num_insts);
                        // bits
                        entropy_ss_site1_bits += prob_1s_i[i] * log((double) 1.0 / prob_1s_i[i]) / log((double) 2.0);
                    }
                    if (prob_1s_j[i] != 0.0) {
                        // mers
                        entropy_ss_site2_mers +=
                                prob_1s_j[i] * log((double) 1.0 / prob_1s_j[i]) / log((double) num_insts);
                        // bits
                ...
                for (int i = 0; i < num_insts; i++) {
                    for (int j = 0; j < num_insts; j++) {
                        ...
                        if (prob_2s[i][j] != 0.0) {
                            ...
                            entropy_ds_mers +=
                                    prob_2s[i][j] * log((double) 1.0 / prob_2s[i][j]) / log((double) num_insts);
                ...
                fp_2s->Write(mutual_information_mers,
                             "Mutual Information (MERS), Site 1 & 2 -- TWO site mut-sel balance");
                fp_2s->Endl();
...
void cAnalyze::AnalyzePopComplexity(cString cur_string) {
    ...
    const int num_insts = m_world->GetHardwareManager().GetInstSet(
            genotype->GetGenome().Properties().Get("instset").StringValue()).GetSize();
        ...
        if (true) {
            ...
            for (int i = 0; i < max_depth; i++) {
                if ((other_genotype = next_it.Next()) != NULL &&
                    ...
                    HashPropertyMap props;
                    cHardwareManager::SetupPropertyMap(props, (const char *) is.GetInstSetName());
                    Genome rev_genome(is.GetHardwareType(), props,
                                      GeneticRepresentationPtr(new InstructionSequence((const char *) reverted)));
...
void cAnalyze::BatchCompete(cString cur_string) {
    ...
    if (m_world->GetVerbosity() >= VERBOSE_ON) {
        cout << "Compete " << batch_size << " organisms from batch " << batch_from << " to batch " << batch_to << ";"
             << endl;
    ...
    /* Create scheduler. */
    Apto::SmartPtr<Apto::Random> rng(
            new Apto::RNG::AvidaRNG(m_world->GetRandom().GetInt(m_world->GetRandom().MaxSeed())));
   ...
   */
    ...
    for (int i = 0; i < batch_size; /* don't increment i yet */) {
        ...
        if (array_pos < 0) {
            cout << "Warning: No organisms in origin batch have positive fitness, cannot sample to destination batch."
                 << endl;
            break;
        }
        ...
        const cInstSet &inst_set = m_world->GetHardwareManager().GetInstSet(
                child_genome.Properties().Get("instset").StringValue());
...
int cAnalyze::LoadCommandList(cInitFile &init_file, tList<cAnalyzeCommand> &clist, int start_at) {
    for (int i = start_at; i < init_file.GetNumLines(); i++) {
        ...
        if (command == "END") {
            ...
            i = LoadCommandList(init_file, *(cur_command->GetCommandList()),
                                i + 1); // Start processing at the next line
        } else {
...
void cAnalyze::PopCommonCPUTestParameters(cWorld *in_world, cString &cur_string, cCPUTestInfo &test_info,
                                          cResourceHistory *in_resource_history, int in_resource_time_spent_offset) {
    Apto::Array<int> manual_inputs;  // Used only if manual inputs are specified
    cString msg;                // Holds any information we may want to send the driver to display
    int use_resources = (cur_string.GetSize()) ? cur_string.PopWord().AsInt() : 0;
    int update = (cur_string.GetSize()) ? cur_string.PopWord().AsInt() : -1;
...
void cAnalyze::SetupCommandDefLibrary() {
  ...
  AddLibraryDef("GET_SKELETONS_EXPANDED", &cAnalyze::GetSkeletons_Expanded); // This is an exercise in how to add analysis functions (Alex); -- Oh how I miss Python.
