...
public:
    cActionLoadPopulation(cWorld *world, const cString &args, Feedback &) : cAction(world, args), m_filename(""),
                                                                            m_update(-1), m_cellid_offset(0),
                                                                            m_lineage_offset(0), m_load_groups(0),
                                                                            m_load_birth_cells(0), m_load_avatars(0),
                                                                            m_load_rebirth(0), m_load_parent_dat(0),
                                                                            m_load_traceq(0) {
        cString largs(args);
    ...
    GetDescription() { return "Arguments: <cString fname> [int update=-1] [int cellid_offset=0] [int lineage_offset=0] [bool load_groups=0] [bool load_birth_cells=0] [bool load_avatars] [bool load_rebirth] [bool load_parent_dat] [int load_traceq]"; }
    ...
    void Process(cAvidaContext &ctx) {
        ...
        if (!m_world->GetPopulation().LoadPopulation(m_filename, ctx, m_cellid_offset, m_lineage_offset, m_load_groups,
                                                     ...
                                                     m_load_parent_dat, m_load_traceq)) {
...
public:
    cActionSavePopulation(cWorld *world, const cString &args, Feedback &feedback)
            : cAction(world, args), m_filename(""), m_save_historic(true), m_save_group_info(false),
              m_save_avatars(false), m_save_rebirth(false), m_plain_name(false) {
        cArgSchema schema(':', '=');
        ...
        schema.AddEntry("plain_name", 4, 0, 1, 0);
    ...
    static const cString
    GetDescription() { return "Arguments: [string filename='detail'] [boolean save_historic=1] [boolean save_groups=0] [boolean save_avatars=0] [boolean save_rebirth=0] [boolean plain_name=0]"; }
    ...
    void Process(cAvidaContext &) {
        ...
        if (m_plain_name) {
            filename = cStringUtil::Stringf("%s.spop", (const char *) m_filename);
        }
...
public:
    cActionLoadStructuredSystematicsGroup(cWorld *world, const cString &args, Feedback &feedback) : cAction(world,
                                                                                                            args),
                                                                                                    m_filename(""),
                                                                                                    m_role("") {
        cArgSchema schema(':', '=');
...
public:
    ...
    void Process(cAvidaContext &) {
        ...
        if (m_filename.GetSize())
            filename = cStringUtil::Stringf("%s-%s", (const char *) m_filename, (const char *) filename);
