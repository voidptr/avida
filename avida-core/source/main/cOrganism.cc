...
void cOrganism::DoOutput(cAvidaContext& ctx, const int value)
{
  ...
  // if we're using avatars, we're using the avatar do output function
  if (m_world->GetConfig().USE_AVATARS.Get()) doAVOutput(ctx, m_input_buf, m_output_buf, false, false);
  // otherwise, normal doOutput
...
void cOrganism::SimOutput(cAvidaContext& ctx, const int value, cContextPhenotype* context_phenotype) {
  // Simulate output
                         ...
                         cContextPhenotype* context_phenotype)
{
  ...
  // reset resource change (deme and global) arrays to 0
  ...
  // create task context from organism (structure with all things relevant to tasks for organism)
  ...
  // combine global and deme resource changes
    ...
    for (int i=0; i<cell_id_lists.GetSize(); i++)
    {
      ...
      if (cell_id_lists[i].GetSize()) {
        ...
        if (j==cell_id_lists[i].GetSize())
          // ??? I"m not sure if this alters things?
          globalAndDeme_resource_count[i]=0;
      }
    }
  }

  // Given current Avida context, task context, global and deme resource counts, current R bins available,
  //   global and deme resource deltas, inst_triggered (output array?), is_parasite, and context phenotype
  //   Return true if task completed, false otherwise; also do magic
  // for now, consider context phenotype to be false
